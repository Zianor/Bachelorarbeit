...
Da in dieser Arbeit Methoden des Maschinellen Lernens verwendet werden, wird im Folgenden eine Übersicht über seine Prinzipien, gängige Techniken und Evaluationsmetriken und verschiedene Lernmodelle und deren mathematischen Hintergrund gegeben.
	...
	\subsection{Lernmodelle und deren mathematischer Hintergrund}
		...
		Es gibt eine Vielzahl von Modellen des maschinellen Lernens, von denen eine Auswahl in dieser Arbeit betrachtet und vorgestellt wird. Alle verallgemeinern eine Verteilung von Trainingsdaten und haben das Ziel, eine Funktion zu finden, mit der $y$ ermittelt werden kann. Bei einer Klassifikation soll sie die verschiedenen Klassen möglichst genau separieren und bei einer Regression den Wert möglichst genau vorhersagen. Diese Funktion wird auch Entscheidungsfunktion genannt und minimiert eine gewählte Kostenfunktion, die beschreibt wie sehr die Vorhersagen von der Zielgröße abweichen.
		
		Grundsätzlich wird zwischen linearen und nicht linear zusammenhängenden bzw. separierbaren Daten unterschieden. Um die Grundprinzipien von Regression und Klassifikation darzustellen, wird für beide zunächst die Funktionsweise eines einfaches lineares Modell, das heißt die Daten werden linear kombiniert, vorgestellt. Das einfachste Modell für die Klassifikation von linear separierbaren Daten ist ein Perzeptron. Seine Idee besteht darin, die Komponenten des Merkmalsverktors $x \in X = \mathbb{R}^n$ unterschiedlich zu gewichten. Hierfür wird ein Gewichtungsvektor $w \in \mathbb{R}^n$ verwendet. Die Ausgabe ist ein binärer Klassifikator $y \in Y = \{-1, 1\}$.  Es soll nun ein Schwellwert $b$, auch Bias genannt, und $w_i$ ermittelt werden, mit dem gilt:		
		\[
			\begin{gathered}
			...
			\end{gathered}
		\]
		Wenn nun $w_0 = b$ und $x_0 = 1$ gewählt werden, ergibt sich durch Umformem als sogenannte Aktivierungsfunktion die Signumfunktion $h(x) = sign(w^Tx)$. Für die Ermittlung von $w$ müssen nun die Kosten minimiert werden. Bei einem Perzeptron entspricht die Kostenfunktion der Anzahl der falschen Klassifikationen. Die Signumfunktion hat den Nachteil, dass sie einen harter Schwellwert besitzt und alle Fehlklassifikation gleich bewertet, unabhängig davon wie nah $\sum_{i=1}^{n} w_i \cdot x_i$ an $b$ ist. Aus diesem Grund werden in der Praxis meist andere Aktivierungsfunktionen gewählt.
		
		Bei einer linearen Regression wird eine lineare Funktion $h(x) = w^Tx$ gesucht, die die Datenpunkte in die reellen Zahlen abbildet. Auch hier wird das $w_i$ gesucht, dass die Kosten minimiert. Die Kostenfunktion leitet sich aus der Methode der kleinsten Quadrate ab bei der die Summe der Fehlerquadrate minimiert wird:
		\[
			\text{cost} = \sum_{i=1}^{n} (y_i - h(x_i))^2
		\]
	\begin{itemize}
	...
	\end{itemize} 
		...
		\begin{itemize}
			...
			\item bei \ac{SVM}s oft oben schon erwähnter Kernel-Trick, also Transformation des Skalarprodukts, damit nicht-lineare Separierung möglich
		\end{itemize}
